...
from mdp_python_algo.robot_pathfinder.config.constants import Moves, RobotConst, ArenaConst
...
def run_simple_hybrid_astar(board):
    ...
    while not open_queue.empty():
        ...
        for neighbor_cell_pos, neighbor_move in neighbor_info:
            neighbor_cell_x = neighbor_cell_pos.cell_x
            neighbor_cell_y = neighbor_cell_pos.cell_y
            neighbor_facing = neighbor_cell_pos.facing
...
def get_neighbors(board, node) -> List[Tuple[CellPosition, Moves]]:
    ...
    for move in Moves:
        neighbor_x, neighbor_y = RobotConst.MOVES_DXY[move][node_facing]
        neighbor_cell_x = node_cell_x + neighbor_x
        neighbor_cell_y = node_cell_y + neighbor_y
        if (ArenaConst.NUM_COLS <= neighbor_cell_x or ArenaConst.NUM_ROWS <= neighbor_cell_y or neighbor_cell_x < 0 or neighbor_cell_y < 0 ):
            continue
        valid = True
        for dx, dy in RobotConst.MOVES_VALID_CHECK_DXY[move][node_facing]:
            step_cell_x = dx + node_cell_x
            step_cell_y = dy + node_cell_y 
